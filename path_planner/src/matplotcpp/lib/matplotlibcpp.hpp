...
template<typename NumericX, typename NumericY>
bool stem(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::string& s = "") //棉棒图
{
    assert(x.size() == y.size());
...
template<typename NumericX, typename NumericY>
bool semilogx(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::string& s = "")  //半对数图
{
    assert(x.size() == y.size());
...
template<typename Numeric>
inline void xticks(const std::vector<Numeric> &ticks, const std::vector<std::string> &labels = {}, const std::map<std::string, std::string>& keywords = {})// 改坐标轴名字
{
...
template <typename Numeric> inline void margins(Numeric margin)    //缩放坐标轴
{
    // construct positional args
    PyObject* args = PyTuple_New(1);
...
inline void tick_params(const std::map<std::string, std::string>& keywords, const std::string axis = "both")//刻度线标签和网格线的外观
{
  detail::_interpreter::get();
