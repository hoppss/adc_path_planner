...
std::pair<double, double> ReedShepp::calc_tau_omega(const double u,
                                                    ...
                                                    const double eta,
                                                    const double phi) {
  double delta = common::NormalizeAngle(u - v);
  ...
  double tau = 0.0;
  if (t2 < 0) {
    tau = common::NormalizeAngle(t1 + M_PI);
  } else {
    tau = common::NormalizeAngle(t1);
  }
  double omega = common::NormalizeAngle(tau - u + v - phi);
...
void ReedShepp::LSL(const double x, const double y, const double phi,
  ...
  std::pair<double, double> polar =
      common::Cartesian2Polar(x - std::sin(phi), y - 1.0 + std::cos(phi));
  double u = polar.first;
  double t = polar.second;
  double v = 0.0;
  if (t >= 0.0) {
    v = common::NormalizeAngle(phi - t);
    if (v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LSR(const double x, const double y, const double phi,
  ...
  if (u1 >= 4.0) {
    ...
    t = common::NormalizeAngle(t1 + theta);
    v = common::NormalizeAngle(t - phi);
    if (t >= 0.0 && v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LRL(const double x, const double y, const double phi,
  ...
  if (u1 <= 4.0) {
    ...
    t = common::NormalizeAngle(t1 + 0.5 * u + M_PI);
    v = common::NormalizeAngle(phi - t + u);
    if (t >= 0.0 && u <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::SLS(const double x, const double y, const double phi,
                    RSPParam* param) {
  double phi_mod = common::NormalizeAngle(phi);
  double xd = 0.0;
  double u = 0.0;
  double t = 0.0;
  double v = 0.0;
...
void ReedShepp::LRSR(const double x, const double y, const double phi,
  ...
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
  std::pair<double, double> polar = common::Cartesian2Polar(-eta, xi);
  ...
  if (rho >= 2.0) {
    t = theta;
    u = 2.0 - rho;
    v = common::NormalizeAngle(t + 0.5 * M_PI - phi);
...
void ReedShepp::LRSL(const double x, const double y, const double phi,
  ...
  double xi = x - std::sin(phi);
  double eta = y - 1.0 + std::cos(phi);
  std::pair<double, double> polar = common::Cartesian2Polar(xi, eta);
  double rho = polar.first;
  double theta = polar.second;
  double r = 0.0;
  double t = 0.0;
  ...
  if (rho >= 2.0) {
    ...
    u = 2.0 - r;
    t = common::NormalizeAngle(theta + std::atan2(r, -2.0));
    v = common::NormalizeAngle(phi - 0.5 * M_PI - t);
    if (t >= 0.0 && u <= 0.0 && v <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  for (size_t i = 0; i < px.size(); ++i) {
    ...
    shortest_path->phi.push_back(
        common::NormalizeAngle(pphi.at(i) + start_node->GetPhi()));
  }
