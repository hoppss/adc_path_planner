...
std::vector<std::shared_ptr<Node2d>> GridSearch::GenerateNextNodes(
  ...
  double current_node_x = current_node->GetGridX();  // grid index
  double current_node_y = current_node->GetGridY();
  double current_node_path_cost = current_node->GetPathCost();
  double diagonal_distance = std::sqrt(2.0) * xy_grid_resolution_;
  std::vector<std::shared_ptr<Node2d>> next_nodes;
  std::shared_ptr<Node2d> up = std::make_shared<Node2d>(
      current_node_x, current_node_y + 1, XYbounds_);
  up->SetPathCost(current_node_path_cost + xy_grid_resolution_);
  std::shared_ptr<Node2d> up_right = std::make_shared<Node2d>(
      current_node_x + 1, current_node_y + 1, XYbounds_);
  up_right->SetPathCost(current_node_path_cost + diagonal_distance);
  std::shared_ptr<Node2d> right =
      std::make_shared<Node2d>(current_node_x + 1,
                               current_node_y, XYbounds_);
  right->SetPathCost(current_node_path_cost + xy_grid_resolution_);
  ...
  std::shared_ptr<Node2d> down = std::make_shared<Node2d>(
      current_node_x, current_node_y - 1,
      XYbounds_);
  down->SetPathCost(current_node_path_cost + xy_grid_resolution_);
  ...
  std::shared_ptr<Node2d> left =
      std::make_shared<Node2d>(current_node_x - 1,
                               current_node_y, XYbounds_);
  left->SetPathCost(current_node_path_cost + xy_grid_resolution_);
